    ...
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        const string sql = """
            ...
            CREATE TABLE IF NOT EXISTS "FriendRequest" (
                ...
                "CreatedAtUtc" timestamp with time zone NOT NULL DEFAULT NOW(),
                CONSTRAINT "CK_FriendRequest_NoSelfReference" CHECK ("FromUsername" <> "ToUsername"),
                CONSTRAINT "FK_FriendRequest_FromUser" FOREIGN KEY ("FromUsername") REFERENCES "User" ("Username") ON DELETE CASCADE,
                CONSTRAINT "FK_FriendRequest_ToUser" FOREIGN KEY ("ToUsername") REFERENCES "User" ("Username") ON DELETE CASCADE
            );

            CREATE UNIQUE INDEX IF NOT EXISTS "IX_FriendRequest_FromUsername_ToUsername"
                ON "FriendRequest" ("FromUsername", "ToUsername");
        """;
        ...
        {
            logger.LogWarning(ex, "Friendship tables could not be initialized automatically.");
        }
    }
